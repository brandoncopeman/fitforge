...
function movingAverage(
  logs: { weight_kg: number; log_date: string }[],
  index: number,
  windowSize = 7
) {
  const slice = logs.slice(Math.max(0, index - windowSize + 1), index + 1);
  if (slice.length === 0) return null;

  const total = slice.reduce((sum, log) => sum + Number(log.weight_kg), 0);
...
export default function MacrosClient({
  ...
  const [showWeightOnHome, setShowWeightOnHome] = useState(
    profile.show_weight_on_home || false
  );
  ...
  const weightChartData = sortedWeightLogs.map((log, index) => ({
    date: formatChartDate(log.log_date),
    weight: Number(log.weight_kg),
    average: movingAverage(sortedWeightLogs, index),
  }));

  const newestSeven = sortedWeightLogs.slice(-7);
  const previousSeven = sortedWeightLogs.slice(-14, -7);

  const newestAverage =
    newestSeven.length > 0
      ? newestSeven.reduce((sum, log) => sum + Number(log.weight_kg), 0) /
        newestSeven.length
      : null;

  const previousAverage =
    previousSeven.length > 0
      ? previousSeven.reduce((sum, log) => sum + Number(log.weight_kg), 0) /
        previousSeven.length
      : null;

  const weeklyRate =
    newestAverage !== null && previousAverage !== null
      ? newestAverage - previousAverage
      : null;
  ...
  const calendarDays = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const log = weightLogs.find((l) =>
        l.log_date?.toString().startsWith(dateStr)
      );
  ...
  async function logWeight() {
    ...
    setWeightLogs((prev) => {
      const existing = prev.findIndex((l) =>
        l.log_date?.toString().startsWith(todayStr)
      );
      if (existing >= 0) {
        const updated = [...prev];
          ...
          <div className="mb-4">
            <div className="flex items-end justify-between mb-1.5">
              ...
              <p className="text-sm text-neutral-400">
                {today.calories} / {profile.daily_calorie_target} kcal
              </p>
            </div>
            <div className="w-full h-2.5 bg-neutral-800 rounded-full overflow-hidden">
              <div
                className={`h-2.5 rounded-full ${
                  today.calories > profile.daily_calorie_target
                    ? "bg-red-500"
                    : "bg-teal-500"
                }`}
                style={{
                  width: `${Math.min(
                    100,
                    (today.calories / profile.daily_calorie_target) * 100
                  )}%`,
                }}
              />
            </div>
        ...
        <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-5 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`w-10 h-5 rounded-full transition-colors relative flex-shrink-0 ${
                  showWeightOnHome ? "bg-teal-600" : "bg-neutral-700"
                }`}
              >
                <span
                  className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${
                    showWeightOnHome ? "left-5" : "left-0.5"
                  }`}
                />
              </button>
            </div>
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1">
              ...
              {weightChange !== null && (
                ...
                >
                  {weightChange > 0 ? "+" : ""}
                  {Number(weightChange).toFixed(1)}kg from start
                </p>
              )}
            </div>
            <div className="flex items-center gap-2">
          ...
          {weightChartData.length >= 2 && (
            <div className="mb-4 rounded-xl bg-neutral-800 p-3">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-xs text-neutral-500">Weight Trend</p>
                  {weeklyRate !== null && (
                    <p
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={weightChartData}>
                    ...
                    {profile.goal_weight_kg && (
                      <ReferenceLine
                        y={Number(profile.goal_weight_kg)}
                        stroke="#14b8a6"
                        strokeDasharray="4 4"
                      />
                    )}
              ...
              <div className="flex items-center justify-center gap-4 mt-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-neutral-500">7-day avg</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-7 gap-1 mb-1">
              {DAYS.map((d) => (
                <p key={d} className="text-xs text-neutral-600 text-center">
                  {d[0]}
                </p>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-1">
              {days.map((day, i) => {
                ...
                if (hasWeight && maxWeight > minWeight) {
                  const ratio =
                    (weightNum - minWeight) / (maxWeight - minWeight);
                  if (profile.goal === "cut") {
                    dotColor =
                      ratio < 0.33
                        ? "bg-teal-400"
            ...
            <div className="space-y-0 mb-4">
              ...
              {weightLogs.slice(0, 5).map((log, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex items-center justify-between py-2 border-t border-neutral-800 first:border-0"
                  >
                    <p
                      className={`text-sm ${
                        isToday ? "text-teal-400" : "text-neutral-400"
                      }`}
                    >
                      {isToday
                        ? "Today"
          ...
          <div className="pt-3 border-t border-neutral-800">
            ...
            {editingGoalWeight ? (
              ...
              <div className="space-y-2">
                ...
                {latestWeight &&
                  (() => {
                    const weeks = estimateWeeksToGoal(
                      Number(latestWeight),
                      Number(goalWeight),
                      caloricDiff
                    );
                    ...
                    return (
                      <div className="bg-neutral-800 rounded-xl p-3">
                        <p className="text-xs text-neutral-500 mb-1">
                          Estimated time at current deficit
                        </p>
        ...
        <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-5">
                  ...
                  {ACTIVITY_LEVELS.map((a) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                        activity === a.value
                          ? "bg-teal-600/20 border border-teal-500 text-white"
          ...
          {!showRecalc && (
            <p className="text-neutral-500 text-xs">
              Update your stats to recalculate your daily calorie and macro
              targets.
            </p>
          )}
        </div>
